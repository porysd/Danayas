...
const updatePaymentHandler = async (paymentId) => {
  await paymentStore.updatePayment(paymentId);
};
...
// Paginator or pagination of the tables
const firstVoid = ref(0);
const rowsVoid = ref(10);

const paginatedVoid = computed(() => {
  return filteredVoid.value.slice(
    firstVoid.value,
    firstVoid.value + rowsVoid.value
  );
});

const onPageChangeVoid = (event) => {
  firstVoid.value = event.first;
  rowsVoid.value = event.rows;
};
  ...
  <main class="customerM">
    ...
    <div class="container">
      ...
      <div class="tabPayBill">
        <Tabs value="0">
          <TabList>
            <Tab value="0">VALID</Tab>
            <Tab value="1">VOIDED</Tab>
          </TabList>
          <TabPanels>
            <TabPanel value="0">
                ...
                <table class="dTable">
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="w-[5%]">{{ payment.transactionId }}</td>
                      <td class="w-[15%]">
                        {{ getBookingName(payment.transactionId) }}
                      </td>
                      <td class="w-[10%]">
                      ...
                      <td class="w-[10%]">{{ formatDates(payment.paidAt) }}</td>
                      <td class="w-[3%]" @click.stop>
                        <T3ButtonTransaction
                          :payment="payment"
                          :showPay="false"
                          :bookingName="getBookingName(payment.transactionId)"
