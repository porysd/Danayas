...
onMounted(async () => {
  try {
    ...
    const response = await fetch(`http://localhost:3000/payments?limit=${limit}&page=${page}`);
...
const updatePaymentHandler = async (payment) => {
  try {
    const response = await fetch(`http://localhost:3000/payments/${payment.paymentId}`, {
      method: 'PATCH',
      headers: {
