...
import T3ButtonFAQs from "../components/T3ButtonFAQs.vue";
import AddButtonFAQs from "../components/AddButtonFAQs.vue";
...
onMounted(() => {
  faqsStore.fetchAllFAQs();
});

const totalFaqs = computed(() => filteredFaqs.value.length);

const addFAQsHandler = async (faqsData) => {
  await faqsStore.addFaqs(faqsData);
};

const updateFAQHandler = async (updatedFaq) => {
  console.log("Received FAQ for Update:", updatedFaq);
  await faqsStore.updateFaqs(updatedFaq); // Call the store's update method
};

const deleteFAQHandler = async (faq) => {
  await faqsStore.deleteFaqs(faq);
};

//FAQs Details
const selectedFaq = ref(null);
const faqDetails = ref(false);

const openFaqsDetails = (faq) => {
  selectedFaq.value = faq;
  faqDetails.value = true;
};
...
<template>
  <main class="employeeM bg-[#EEF9EB] dark:bg-[#09090b]">
    ...
    <div class="container">
      <div class="headers">
        <h1 class="text-5xl font-black">Frequently Asked Questions (FAQs)</h1>
      ...
      <div class="searchB">
        ...
        <div class="empBtns">
          <AddButtonFAQs class="addBtn" data="FAQs" @addFAQs="addFAQsHandler" />
        </div>
      </div>
      ...
      <div class="tableContainer">
        <table class="dTable">
          ...
          <tbody>
            ...
            >
              <td class="w-[3%]">{{ faq.faqsId }}</td>
              <td class="w-[30%]">
                {{ faq.question }}
              </td>
              <td class="w-[50%]">{{ faq.answer }}</td>
      ...
      <div class="modal-content font-[Poppins]">
        ...
        <div class="flex flex-col gap-2">
          ...
          <p>
            ...
            {{ formatDates(selectedFaq?.createdAt) }}
          </p>
          <p>
            <strong>Updated At:</strong>
            {{ formatDates(selectedFaq?.updatedAt) }}
          </p>
        </div>
        <Divider />
